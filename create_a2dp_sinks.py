...
def add_bluetooth_audio_source(path, retry_timeout=1, retries=0, give_up=5):
    if retries == give_up:
        return
    ...
    journal.send("Found: %s attempting to create interface" % bt_addr)
    if os.system(cmd % (bt_addr, expected_sink)) != 0:
        journal.send("Failed to add loopback for %s. Waiting: %ds" % (bt_addr, retry_timeout))
...
if __name__ == '__main__':
    journal.send("Starting to automatically monitor for BT Audio sources")
    dbus.mainloop.glib.DBusGMainLoop(set_as_default=True)

    bus = dbus.SystemBus()

    journal.send("Automatically creating a2dp sinks from BT Audio sources")
